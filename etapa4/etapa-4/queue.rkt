...
(define (enqueue x q)
  (define q_nou (struct-copy queue q [right (cons x (queue-right q))]
                             [size-r (add1 (queue-size-r q))]))
  (if (> (queue-size-r q_nou) (queue-size-l q_nou))
      (struct-copy queue q_nou [right '()] [size-r 0] [size-l (+ (queue-size-r q_nou) (queue-size-l q_nou))] [left (rotate (queue-left q_nou) (queue-right q_nou) empty-stream)])
      q_nou))
...
(define (dequeue-help q)
  (struct-copy queue q [left (if (> 2 (queue-size-l q))
                                 null
                                 (stream-rest (queue-left q)))]
               [size-l (sub1 (queue-size-l q))]))
(define (dequeue q)
  (define q_nou (dequeue-help q))
  (if (> (queue-size-r q_nou) (queue-size-l q_nou))
      (struct-copy queue q_nou [right '()] [size-r 0] [size-l (+ (queue-size-r q_nou) (queue-size-l q_nou))] [left (rotate (queue-left q_nou) (queue-right q_nou) empty-stream)])
      q_nou))
