...
(define (add-to-counter C name n-items)
  ...
  (define y (list x))
  (define total (counter-tt (tt+ C n-items)))
  (define C_nou (struct-copy counter C [queue (append (counter-queue C) y)] [tt total]))
  C_nou)
...
(define (serve requests C1 C2 C3 C4)
  ...
  (define (persoana_noua name n-items)
    (define x (if (> n-items ITEMS)
        (min-tt (list C2 C3 C4))
        (min-tt (list C1 C2 C3 C4))))
    (cond
      ((= (counter-index C1) (car x)) (serve (cdr requests) (add-to-counter C1 name n-items) C2 C3 C4))
      ((= (counter-index C2) (car x)) (serve (cdr requests) C1 (add-to-counter C2 name n-items) C3 C4))
      ((= (counter-index C3) (car x)) (serve (cdr requests) C1 C2 (add-to-counter C3 name n-items) C4))
      ((= (counter-index C4) (car x)) (serve (cdr requests) C1 C2 C3 (add-to-counter C4 name n-items))))
     )
  
  (define (delay_func index minutes)
    (cond
  ...
  (if (null? requests)
      (list C1 C2 C3 C4)
      (match (car requests)
        [(list 'delay index minutes) (delay_func index minutes)]
        [(list name n-items)         (persoana_noua name n-items)])))
